...
construction; ogg is lossy and does not round-trip. The rule is what
...
stream, ogg does not, because its bitstream state spans the whole stream. The
...
and ignored; `response_format` defaults to `wav` because no mp3 encoder
ships; `mp3`, `aac` and `opus` are refused by name (`opus` because the
`ogg` here is Vorbis, which shares a media type with Opus and no
bitstream); a `speed` outside this engine's range is refused rather than
