...
`auto` resolves to `eager` on MPS, where the fused scaled-dot-product path
aborts the whole interpreter (`LLVM ERROR: Failed to infer result type(s)`
from `mps_matmul`, no Python traceback), and on NVIDIA GPUs older than
