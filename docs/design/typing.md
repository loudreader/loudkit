...
- **Manifest reads are checked, not assumed.** A checkpoint manifest is
  parsed JSON (`Mapping[str, object]`). Small helpers narrow the values
  (`_block` / `_number` in `manifest.py`, `_cfg_int` / `_cfg_float` in
...
`pyproject.toml` carries two `ignore_missing_imports` overrides, reserved for
packages that genuinely ship no stubs / `py.typed` (verified per package). The
first is `librosa` alone, which also sets `follow_imports = "skip"` because
librosa 1.0's own type information does not parse. The second names `torch`,
`torchaudio`, `safetensors`*, `soundfile`, `librosa`, `onnxruntime`,
`coremltools`, `mcp`, `grpc`, `grpc_tools`, `tokenizers` and
`huggingface_hub`. Values arriving from them are `Any`, and the call site pins
them down rather than letting them spread:
...
- **coremltools** enters through exactly one `cast`, in
  ...
  `MLModel` in `_PinnedInputs` and hands back that protocol, so nothing
  downstream ever holds the untyped object.
...
- **fastapi / uvicorn / pydantic are *not* in the override.** They ship real
  type information, so `transports/http.py` is checked against their actual
  APIs. They
...
| `execution.ExecutionConfig.merged` | `arg-type` | the same `replace` gap on the execution side, where the merge builds its `changes` map as `dict[str, object]`. |
| `cli._SafetensorError` | `no-redef` | the fallback class stands in when the `safetensors` import guard fires, so `--help` still parses. Two definitions of one name is what a guard looks like. |
...
| `models/enroll.py` (`librosa.filters`, `librosa.effects`, x2) | `attr-defined` | librosa's stubs are skipped wholesale by the override above, so mypy knows the package but not that it re-exports its submodules. The `import librosa.filters` a line earlier is what makes the attribute exist at runtime. |
...
- `engine.py` used `np.concatenate` and `Iterator` without importing either.
  The long-form path raised `NameError` on any text long enough to split.
...
- Fourteen suites run under `tools/mypy-tests.ini`, on `tools/mypy.ini`'s
  flags: everything that decides what a user downloads and what the five ports
  have to agree about. A type error in a test of that surface hides a test that
  passes while asserting nothing. The list is explicit and grows a module at a
  time; the rest of `tests/` — the engine, model, server, MCP, CLI and backend
  suites — is not checked, because covering it means annotating rather than
  checking, and a gate that runs over a fifth of the suite is worth more than a
  plan to cover all of it.
- The suites carry no `# type: ignore` comments, checked or not. Outside a
  profile a suppression reads like a check that does not run and stands ready
  to hide the error it names; inside one, the error it names should be fixed.
  `test_hygiene.py` holds the suites to that.
- `tools/` and `research/` run under their own profile, `tools/mypy.ini`. It
  allows untyped defs, untyped globals and untyped calls, because these are
  scripts with argparse mains and tracer-fed `forward` methods. Everything else
  it holds: bodies are checked, imports are followed, optionals are strict, and
  a suppression that suppresses nothing is an error. `tools/release/`, which
  builds what users download, is held to the flags `--strict` implies.
