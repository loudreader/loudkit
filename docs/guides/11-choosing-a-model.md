...
Start with **loudr-1**. Try **loudr-1-turbo** when you want audio sooner.
Compare them on your own text and voice before deciding: they produce
different readings, even with the same seed.
...
Both models run on every runtime:
...
[Benchmarks](../benchmarks.md) has the measured speed of both models per device.
