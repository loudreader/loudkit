...
enum ManifestReader {
    ...
    ///
    /// Only an absent key takes that default: a key that is not a mapping, an
    /// explicit `null` included, is refused rather than skipped, so a block the
    /// manifest declares cannot fall back to the shipping defaults unnoticed.
    ...
    /// `encoding/json`, `serde_json` and `JSON.parse` all keep the last. Left
    /// alone, a manifest carrying the same key twice runs this engine on one
    /// value and the other four on another from identical bytes, and because
    /// each port hashes the value it read, the fingerprints agree about a
    /// disagreement.
    ///
    ...
    /// is handed over untouched. The scan's only job is finding duplicates:
    /// parsing and validating stay Foundation's, and a document the scan
    /// cannot follow is passed through for Foundation to refuse.
    ///
    /// `what` is what the caller was reading, and it is not optional because a
    /// caller that cannot name it has nothing to put in the message: a refusal
    /// names the file and the part of it being read, which Foundation's own
    /// parse failure does not. The other ports word it the same way.
    ///
