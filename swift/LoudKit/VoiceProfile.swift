...
public struct VoiceProfile: Sendable {
    ...
    ///
    /// A present value of another type is refused, not defaulted: `language: 5`
    /// took English here, and language selects the text funnel, so a voice
    /// whose header says 5 was read through whichever funnel the fallback
    /// happened to land on. `VoiceProfile._header_str` refuses it in this
    /// sentence.
    ///
