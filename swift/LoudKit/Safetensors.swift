...
public final class Safetensors {
    ...
    public init(url: URL) throws {
        ...
        // traps for anything >= 2^63, so the refusal below has to happen in
        // the wider type: converting first would trap inside the check meant
        // to catch it. This reader is reached by `VoiceProfile.load`, i.e. by
        // any downloaded `.voice.safetensors`, so the header is untrusted.
    ...
    public func ints(_ name: String) throws -> [Int] {
        let (info, bytes) = try raw(name)
        switch info.dtype {
        // `loadUnaligned` here for the same reason as `floats` above: a
        // tensor starts wherever the header ends, so the slice carries no
        // alignment guarantee and `bindMemory` requires one. `bindMemory`
        // happens to work on ARM and is undefined behaviour by Swift's own
        // rules, the optimiser is entitled to assume the alignment it was
        // promised, and a safetensors file is data from outside the process.
        case "I64":
            return bytes.withUnsafeBytes { buf in
