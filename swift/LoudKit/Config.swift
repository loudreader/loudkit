...
public struct AlgorithmConfig: Sendable, Equatable {
    ...
    public static func fromManifest(_ manifest: [String: Any]) throws -> AlgorithmConfig {
        ...
        if !defaults.isEmpty {
            ...
            // The same range check, and the one that was missing: `Engine`
            // refuses a caller's `maxNewTokens` below 1 and said nothing about
            // the manifest's own. Zero generated an empty row and returned it
            // as a result; a negative reached `0..<cap` in the token generator.
            // Python, Go, Rust and JS all refuse the manifest here, so a pack
            // that loads on four ports and not the fifth is the drift this
        ...
        // `window` or `eos_floor` is the same ragged default.
        //
        // A guard stood here refusing a manifest that carried neither block,
        // and telling the caller to run tools/amend_manifest.py. It was the
        // only port that refused: `_window_from` returns `WindowConfig()` for
        // an absent key and for an explicit null, and Go, Rust and JS all
        // default the same way. So the five disagreed about which checkpoints
        // load at all, which is a worse failure than the re-guess the guard was
        // written to prevent -- and it is not a guess, because all five spell
        // the same default. Anything else non-null is still refused, by
        // `ManifestReader.block`, exactly as `_window_from` refuses it.
