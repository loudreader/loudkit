...
public struct PostprocessConfig: Sendable, Equatable {
    ...
    /// constant to tune, and a cut that survives it only ever removes a
...
extension Postprocess {
    ...
    /// the listed silence ids, the run has no exit. The sampler's repetition
    /// penalty (``LRSamplerV1/sample(logits:step:seen:)``) closes the trap at
    /// its source; this rule is the detector for what still gets through, and
    /// for any checkpoint or configuration where the trap re-opens. A trapped
    /// row reads `clean` to every other rule in the layer, because no other
    /// rule looks inside the sequence: the rest read the tail, or count the
    /// whole row against its text.
    ///
    /// Three triggers, all integer-exact, any one condemns:
    ///
    /// - **no speech at all**: every generated token is in the
    ///   silence-or-quiet family. A mute row is a seed lottery, so the route
    ///   out is a re-roll rather than a cut.
    ...
    ///   dead air is a stall the cap happened to interrupt. This also closes
    ///   a structural hole: the ceiling clips rows to 4.0x text tokens + 40,
    ///   so past 80 text tokens a cap-hit stall can never reach the 4.5x
    ///   desperation threshold, which puts the rule that means "certainly
    ///   broken" out of reach of the most broken rows this layer sees.
    ///
    ...
    ///
    /// The trap measurements, the specimens and the prevalence of each
    /// trigger are in `docs/design/postprocess-detectors.md`.
