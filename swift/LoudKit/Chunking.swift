...
public enum Chunking {
    ...
    /// one it overshoots -- the same mechanism as every other defect in this
    /// campaign, arriving through the chunker. The arm that measured
    /// punctuation against word boundaries was retracted with the harness that
    /// produced it (it seeded the second half from the next chunk's stream),
    /// and the re-measurement compared only this law against no law, so the
    /// numbers that once appeared here have no un-retracted source. Not sought
    /// is not avoided: the nearest word break can follow a comma.
    ///
