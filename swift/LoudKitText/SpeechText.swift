...
public enum SpeechText {
    ...
    /// prefix pass never sees it and `0.49¢` reaches the clock reader intact.
    ...
    static func speakSymbols(_ text: String, languageId: String?) -> String {
        ...
        // `"50%\u{FE0F}"` does not contain `"%"`. The table then never
        // matches, the punctuation pass turns the unrecognised cluster into a
        // space, and the word it was attached to is deleted from the
        // utterance. U+FE0F is what a chat client or a Markdown document
        // emits for a tick, a cross or an arrow, so this is ordinary pasted
        // text.
        //
    ...
    /// letter numbers; and not `CharacterSet.letters`, which Foundation answers
    /// wrongly above the BMP and which is documented as L* **and M***, so it
    /// calls a combining mark a letter where Python, Go, Rust and JS call it
    /// none.
    ///
    /// L* and *not* M*, and the difference is audible: NFC composes a mark
    /// into its base ahead of this pass, but one that cannot compose reaches
    /// it, and `"Az\u{032C}b"` stays one word where that mark counts as a
    /// letter and reads as `Az b` where it does not. The character censuses
    /// behind both rules are in `docs/design/text-funnel.md`.
    ...
    ///
    /// `\s` names a different set in each of the five implementations, and
    /// the difference is audible: ECMAScript's excludes U+0085 NEL, CPython's
    /// admits U+001C-U+001F. NEL in particular is ordinary in scraped and
    /// epub text, and a separator that survives in one port changes where
    /// that port's reader breathes.
    ///
    /// ICU's `\s` and Foundation's `.whitespacesAndNewlines` both answer
    /// White_Space exactly today, so this table changes no output here. It is
    /// written out anyway because the other four ports name the set they
    /// mean, and a port whose answer depends on which ICU it links against is
    /// a port whose agreement is a coincidence rather than a contract. Python
    /// reference: `loudkit.frontend.speechtext.WHITE_SPACE`.
    ...
    /// included, so all five ports fold the same characters to the same
    /// spellings.
    ///
    /// Neither half is safely computable here. Walking down to a decimal
    /// block's start walks out of the block where two blocks touch:
    /// `MATHEMATICAL DOUBLE-STRUCK DIGIT ZERO` follows `MATHEMATICAL
    /// SANS-SERIF DIGIT NINE` with nothing between. NFKC does not reach
    /// `ETHIOPIC NUMBER TEN`, the Aegean numbers, the Kaktovik digits or the
    /// Meroitic numerals. Asking `generalCategory` *whether* to fold would put
    /// a Unicode version back into an answer the table takes it out of:
