...
public enum Numbers {
    ...
    public static func expandTimes(_ text: String, language: String) -> String {
        ...
        for m in timeRun.matches(in: text, range: NSRange(location: 0, length: ns.length)) {
            ...
            //
            // Every `continue` in this loop belongs above the append below,
            // which is why this one moved: leaving a match takes the run up to
            // it with you, or it is written once here and again by the tail
            // append at the end, and two rejected matches emit it twice over.
    ...
    /// (`Lo`, numeric value 1) and for every `No` and `Nl`. Measured: `24:00.\u{4e00}\u{4e8c}\u{4e09}`
    /// found a "digit" behind the dot, decided the clock was part of a longer
    /// number, and Swift alone read `tjuefire:00` where the other four read the
    /// time.
