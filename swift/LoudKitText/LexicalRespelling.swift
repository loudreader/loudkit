...
/// mangles it. Respelling rather than inline tag switching: "dałnloud" is how
/// the word actually sounds in a Polish sentence, accent included, so the
/// lexicon writes down what a Polish speaker says instead of asking the
/// engine for a language it cannot switch to mid-utterance.
/// `docs/design/text-funnel.md` records the listening test behind that choice.
///
...
public enum LexicalRespelling {
    ...
    ///
    /// Both channels rather than `ChatterboxAssets` alone: `swift test` does
    /// not populate the asset channel, so `Bundle.module` is what makes the
    /// lexicon reachable under test. A lexicon neither channel supplies is an
    /// error and an empty fingerprint, not a quiet degrade; see the loader
    /// below.
    ...
    static func numberWords(_ token: String) -> String? {
        ...
        // which sends the token to the caller's fallback and through an
        // ASCII-keyed table that has no entry for it, so a Polish passage
        // carrying Arabic-Indic numerals reads as if they had never been
        // written. Python reads the same string as "sto dwadzieścia trzy";
    ...
    private static func spelledCodeToken(_ word: String) -> String? {
        ...
        // All or nothing. Skipping a character with no letter name reads
        // `Müller123` as *em el el e er jeden dwa*, the `ü` simply gone, a
        // name changed rather than mispronounced; and truncating at the
        // length cap instead of refusing loses the last digits of
        // `żelazny2024`. A token half-read is worse than one left written,
    ...
    private static func respelled(_ word: String) -> String {
        ...
        if !word.contains(where: { $0.isLetter }) {
            ...
            // Digit by digit, but never to *nothing*. Dropping the
            // characters the ASCII-keyed table does not know deletes an
            // unmappable token from the utterance rather than reading it
            // badly. A word this function cannot improve is returned
