...
# into a failure. A plain skip here is how these 25 tests ran nowhere while the
# `check` job installed a `.[dev]` that carried no grpcio.
...
def _schema(files: dict[str, bytes]) -> dict[str, bytes]:
    ...
    """
    return {name: _GENERATOR_BANNER.sub(b"", data) for name, data in files.items()}
...
def test_ogg_is_unary_only_here_as_it_is_over_http(library) -> None:
    """``proto/loudkit.proto``: a difference from the HTTP contract is a bug.

    The format check was shared with the unary RPC, which accepts every
    encoder, so the streaming RPC delivered ogg one container per chunk while
    ``/v1/synthesize/stream`` refused it -- and the reason is about the
    container, not the door: an Ogg bitstream's state spans the whole stream,
    so a per-chunk container names bytes that are not what they say.
    """
    from loudkit.proto import loudkit_pb2, loudkit_pb2_grpc
    ...
    try:
        stub = loudkit_pb2_grpc.SpeechStub(channel)
        request = loudkit_pb2.SynthesizeRequest(
            text="Hello there.", voice="fake", audio_format="ogg"
        )
