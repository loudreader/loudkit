...
class TestOutputFormats:
    ...
    def test_an_unknown_format_is_a_422_listing_the_real_ones(self, tmp_path) -> None:
        client = self._client(tmp_path)
        resp = client.post(
            "/v1/synthesize", json={"text": "hi", "voice": "fake", "format": "aac"}
        )
        assert resp.status_code == 422
        body = json.dumps(resp.json())
        for name in ("wav", "pcm16", "flac", "ogg", "mp3", "opus"):
            assert name in body, body

    @pytest.mark.parametrize("fmt", ["ogg", "opus", "mp3"])
    def test_the_stream_refuses_a_container_that_spans_the_stream(self, tmp_path, fmt) -> None:
        """Each event must play on its own. An Ogg bitstream's state spans the
        whole stream and each MP3 chunk carries its own encoder delay, so these
        three are a 422 naming the formats that do stream."""
...
class TestTheOpenAICompatibleRoute:
    ...
    def test_an_unset_format_is_wav_not_mp3(self, tmp_path) -> None:
        ...
        OpenAI defaults `response_format` to mp3. This route answers wav, which
        is lossless and which every client decodes; the clients that want mp3
        ask for it.
        """
    ...
    @pytest.mark.parametrize("fmt", ["aac"])
    def test_a_format_this_server_cannot_encode_is_refused_by_name(
        self, tmp_path, fmt: str
    ) -> None:
        # libsndfile has no AAC encoder, so the name is refused rather than
        # answered with some other codec under it.
    ...
    def test_mp3_is_mpeg_audio(self, tmp_path) -> None:
        body = (
            _client(tmp_path)
            .post(
                "/v1/audio/speech",
                json={"input": "hi", "voice": "fake", "response_format": "mp3"},
            )
            .content
        )
        # An ID3 tag, or an MPEG audio frame opening on eleven set sync bits.
        assert body[:3] == b"ID3" or (body[0] == 0xFF and body[1] & 0xE0 == 0xE0)

    @pytest.mark.parametrize(
        ("asked", "codec"), [("opus", b"OpusHead"), ("ogg", b"\x01vorbis")]
    )
    def test_opus_and_ogg_are_each_their_own_codec(self, tmp_path, asked, codec) -> None:
        """`opus` and `ogg` share a container and no bitstream, so each name is
        answered with its own codec or a client decodes the other on a `200`."""
        body = (
            _client(tmp_path)
            .post(
                "/v1/audio/speech",
        ...
        assert codec in body[:128]

    def test_a_codec_the_loaded_libsndfile_lacks_is_refused_by_name(
        self, tmp_path, monkeypatch
    ) -> None:
        """Whether mp3 can be written depends on how the loaded libsndfile was
        built, so a server whose library lacks it says so instead of failing
        inside the encoder."""
        import soundfile as sf

        formats = {k: v for k, v in sf.available_formats().items() if k != "MP3"}
        monkeypatch.setattr(sf, "available_formats", lambda: formats)
        resp = _client(tmp_path).post(
            "/v1/audio/speech",
            json={"input": "hi", "voice": "fake", "response_format": "mp3"},
        )
        # A classified refusal: the request error the one-shot route gives every
        # refusal, in OpenAI's envelope.
        assert resp.status_code == 422
        message = resp.json()["error"]["message"]
    ...
    def test_the_real_openai_sdk_talks_to_it_unmodified(self, tmp_path) -> None:
        ...
        # The format their clients ask for by default comes back as what it
        # says it is.
        mp3 = sdk.audio.speech.create(
        ...
        with pytest.raises(openai.BadRequestError, match="aac"):
            sdk.audio.speech.create(
                model="tts-1", voice="fake", input="hi", response_format="aac"
            )
