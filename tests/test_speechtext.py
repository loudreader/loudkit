...
class TestTheGeneratedLexiconLoadsOnce:
    """`PolishLexicon` used to hold three class-level caches and to `cast` the
    four JSON values unverified, two lines under a comment refusing to do that.
    The load is one `lru_cache` over a frozen record now, and the class is the
    door onto it: it is in `__all__`, so its five classmethods keep answering.
    """
...
class TestTheNumeralTableIsTheOnlyAuthority:
    ...
    def test_the_hashed_data_files_carry_no_prose(self) -> None:
        ...
        `grammar_digest` hashes these files as raw bytes, and both of them used
        to carry an `about` member. Correcting one sentence in it moved the
        grammar digest and the `algorithm_fingerprint` under it, five ports
        re-pinned their copies, and every sample rendered exactly as before —
        which is the opposite of what `COMPATIBILITY.md` promises the
        fingerprint means.

        The prose lives beside the data now: `numbers.about.md` and
