...
A benchmark whose command line is misread reports a measurement of something
other than what was asked for, and says nothing while doing it: an ignored
``--seed`` is a run at the default seed under a command line that names
another. These cases pin the parsers, the defaults, the seed each run actually
uses, and a refusal for each malformed value.
