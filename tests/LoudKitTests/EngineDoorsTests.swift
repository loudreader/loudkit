...
///
/// Without them each of these is a silent success: `synthesizeWindow` on text
/// the funnel empties tokenises the bare language tag and renders babble, a
/// window that generates nothing comes back as an empty row, and
/// `synthesizeTokens` filters a caller's out-of-range ids away and renders what
/// is left.
