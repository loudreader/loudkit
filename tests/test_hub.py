...
def _lay_out_receipt_case(root: Path, case: dict) -> None:
    ...
    # `newline="\n"` on every one: text mode translates on Windows, so a case
    # laid out there wrote CRLF, the SHA256SUMS digest stopped matching the
    # one the receipt records, and the reference judged its own fixture
    # differently from the four ports reading the same bytes.
    if case["receipt"] is not None:
        (root / hub.RECEIPT_NAME).write_text(
            json.dumps(case["receipt"]), encoding="utf-8", newline="\n"
        )
    if case["sums"] is not None:
        (root / "SHA256SUMS").write_text(case["sums"], encoding="utf-8", newline="\n")
...
class TestTheReceiptIsTheSharedContract:
    ...
    def test_a_written_receipt_has_the_fixture_shape(self, tmp_path) -> None:
        ...
        # LF: the digest below is of this string, and text mode would
        # write CRLF on Windows and hash something else.
        (tmp_path / "SHA256SUMS").write_text(sums, encoding="utf-8", newline="\n")
...
class TestALocalDirCarriesAReceipt:
    ...
    def _download(self, monkeypatch, client, target, revision=None):
        monkeypatch.setattr(hub, "_hub", lambda: client)
        return hub.download(self._REPO, local_dir=target, revision=revision)

    def test_offline_refuses_a_commit_the_directory_does_not_hold(
        self, monkeypatch, tmp_path
    ) -> None:
        """A pin is not a preference, and a warning is not a refusal.

        Unreachable hub used to hand back whatever was cached, naming the
        commit in a warning. A build that pins its weights therefore ran on
        different ones and reported success, which is the failure pinning
        exists to prevent.
        """
        ...
        monkeypatch.setattr(hub, "_resolve_commit", lambda *_a, **_k: None)
        with pytest.raises(ValueError, match="pinned revision is not a preference"):
            hub.download(self._REPO, local_dir=target, revision="b" * 40)
        assert hub.download(self._REPO, local_dir=target, revision="a" * 40) == target

    def test_offline_refuses_a_tag_the_directory_was_not_fetched_for(
        self, monkeypatch, tmp_path
    ) -> None:
        """A tag cannot be resolved with no hub, and the receipt still knows
        which one this copy answers for."""
        client = _LocalDirClient("a" * 40, tmp_path / "remote")
        target = tmp_path / "release"
        assert self._download(monkeypatch, client, target, revision="v0.1.1") == target

        monkeypatch.setattr(hub, "_resolve_commit", lambda *_a, **_k: None)
        with pytest.raises(ValueError, match="cannot be resolved without it"):
            hub.download(self._REPO, local_dir=target, revision="v0.2.0")
        assert hub.download(self._REPO, local_dir=target, revision="v0.1.1") == target
