...
def test_eviction_survives_a_concurrent_insert(
    ...
    def parked(profile: VoiceProfile) -> int:
        ...
        # true -- the guard used to be "has the insert landed yet", which is
        # never, because the lock is what this test is about, so every entry in
        # the walk paid the full timeout and the test cost fourteen seconds to
        # prove something it had proved in the first two hundred milliseconds.
