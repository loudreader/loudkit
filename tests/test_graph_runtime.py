...
# Most of this module is arithmetic and needs nothing installed. Three tests
# reach `loudkit.load` or `loudkit.enroll` on a graph device, which refuses
# before any monkeypatch can stand in for it when the runtime is absent, so
# they say what they need rather than failing on a runner without it.
#
# The three select `device="coreml"` explicitly, so `coremltools` is what they
# need, even in an environment where ONNX Runtime is installed.
needs_coreml = pytest.mark.skipif(
    importlib.util.find_spec("coremltools") is None,
    reason="these ask for device='coreml': pip install coremltools",
)
