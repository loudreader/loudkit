...
# tree and reports on code nobody is editing. That is not a subtle wrong answer:
# it is green where the tree under test is red, and red where it is green.
# Four separate runs in one campaign were spent on results produced this way
# before anyone noticed, so the suite says so rather than letting it happen.
_want = Path(__file__).resolve().parent.parent / "python" / "loudkit"
_got = Path(_loudkit.__file__).resolve().parent
if _want.is_dir() and _got != _want:
    raise RuntimeError(
...
def _hold_the_provenance_clock(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    WAV carries a C2PA trailer, and the trailer carries a creation timestamp,
...
def tool(name: str) -> ModuleType:
    ...
    Neither directory is a package, so the suite used to reach them eleven
    different ways: four ``spec_from_file_location`` variants, five
    ``sys.path.insert`` blocks with a ``finally`` pop, and two inserts with no
    pop at all, which left ``tools/`` on the path for everything collected
    afterwards.
    ...
    takes the *script's* own insert instead, which is how the old blocks leaked
    while looking like they cleaned up.
