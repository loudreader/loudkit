...
@pytest.mark.parametrize(
    ("fmt", "media_type"), [("mp3", "audio/mpeg"), ("opus", "audio/ogg; codecs=opus")]
)
def test_synthesize_mp3_and_opus_come_back_as_what_they_say(
    tmp_path, fmt: str, media_type: str
) -> None:
    """What a chat or a phone is sent: the reply names the codec, and the
    bytes are that codec."""
    server = _server(tmp_path)
    result = _run(
        server.call_tool(
            "synthesize",
            {"text": "one. two.", "voice": "fake", "seed": 7, "format": fmt},
        )
    )
    payload = json.loads(result.content[0].text)
    ...
    if fmt == "opus":
        ...
        assert b"OpusHead" in audio[:128]
    else:
...
def test_synthesize_unknown_format_is_refused_with_the_supported_list(tmp_path) -> None:
    ...
    result = _run(
        server.call_tool(
            "synthesize",
            {"text": "one. two.", "voice": "fake", "format": "aac"},
        )
    )
    payload = json.loads(result.content[0].text)
    assert payload["error_kind"] == "bad_request"
    assert "aac" in payload["error"]
...
_REFUSALS = [
    ...
    pytest.param(
        {"text": "hi", "voice": "fake", "format": "aac"}, "invalid_request", id="format"
    ),
...
def test_a_saturated_engine_still_answers_list_voices(tmp_path) -> None:
    ...
    async def drive() -> None:
        ...
        callers = int(pool) + 20
        # Plain tasks rather than `asyncio.TaskGroup`, which is 3.11 and
        # `requires-python` promises 3.10. Nothing here wants a group's
        # cancellation semantics: the calls are awaited below, and the ones
        # still running when the assertions are made are drained after the
        # lock is released so none outlives the test.
        calls = [
