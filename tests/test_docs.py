...
def test_every_port_reads_every_postprocess_field() -> None:
    ...
    A field left out of one of those lists is a manifest key that port does not
    read, and nothing notices while the defaults agree. A checkpoint that sets
    one is all it takes for the manifest to declare one recipe and an engine to
    run another.
