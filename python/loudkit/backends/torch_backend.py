...
def pin_determinism(execution: ExecutionConfig) -> None:
    ...
    # so a non-deterministic engine runs TF32 convolutions with nothing saying
    # so, and an fp32 measurement made under it is neither fp32 nor bit-exact
    # with itself.
    torch.backends.cuda.matmul.allow_tf32 = tf32
    torch.backends.cudnn.allow_tf32 = tf32
    # Symmetric on purpose. These are process-global, so setting them only
    # under `deterministic` leaves a later non-deterministic engine running
    # under whatever an earlier one asked for rather than under its own config.
