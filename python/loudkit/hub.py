...
def write_receipt(root: Path, *, repo: str, revision: str | None, commit: str) -> None:
    ...
    (root / RECEIPT_NAME).write_text(json.dumps(receipt, indent=1) + "\n", encoding="utf-8")
...
def download(
    ...
    *,
    ...
    # one: that raised `NotADirectoryError` from inside the fetch and reached
    # the caller as "the copy may be truncated", advice about a download that
    # had not started.
