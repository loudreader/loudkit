...
class ExecutionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        Every closed field is a ``Literal``, which nothing checks at runtime.
        Without this check a typo such as ``attention='bogus'`` or
        ``precision={'token_generator': 'int8'}`` travels to the backend and
        fails there, naming neither the field nor the config.
        """
    ...
    def resolved_attention(self) -> Literal["eager", "sdpa"]:
        ...
        Either half, not the generator alone: the renderer's flow estimator
        runs attention too, so a generator on the CPU beside a renderer on MPS
        answers ``eager``. The fused path is not portable on Metal, where on
        some macOS builds the shader fails to compile and the call raises out
        of `scaled_dot_product_attention`.
        """
        if self.attention is not None and self.attention != "auto":
            return self.attention
        gen = self.resolved_generator_device()
        if gen == "mps" or self.resolved_renderer_device().split(":", 1)[0] == "mps":
            return "eager"
