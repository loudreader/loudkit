...
class TorchTokenGenerator(nn.Module):
    ...
    def _prefill_embeds(self, text_tokens: NDArray[np.int64], voice: VoiceProfile) -> Tensor:
        ...
        if tt.shape[1] > self.MAX_TEXT_POSITIONS:
            # Checked before the embedding rather than discovered inside it.
            # The table has `MAX_TEXT_POSITIONS` rows, so a longer prompt
            # otherwise surfaces as a bare `IndexError` from a
            # positional-embedding lookup, naming neither the text, the limit,
            # nor the method that exists precisely for this case.
            raise ValueError(
