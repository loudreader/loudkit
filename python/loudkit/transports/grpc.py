...
wav and pcm16 carry exactly this; flac, ogg, mp3 and opus carry less. The bound
below uses the uncompressed figure for all six because a compressed size is not knowable
    ...
    *,
    ...
    class Speech(pb2_grpc.SpeechServicer):  # type: ignore[misc,name-defined]
        ...
        def SynthesizeStream(  # noqa: N802, PLR0912, PLR0915 - one stream: producer, writer, verdict
            ...
            # refuses ogg, opus and mp3 for a reason that is about the
            # container, not the door: an Ogg bitstream's state spans the
            # whole stream and each MP3 chunk carries its own encoder delay,
