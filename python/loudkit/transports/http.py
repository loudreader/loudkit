    ...
    *,
    ...
    async def synthesize(  # noqa: PLR0915 - one linear route; the ladder is the contract
        ...
        try:
            ...
            # The clause the ladder above used to end without. A defect that is
            # not a `ValueError` reached Starlette's own handler, which answers
            # `text/plain` "Internal Server Error" with no `code` and no log
            # line -- for a server that promises a `code` in every JSON error
            # body. The realistic one is `Engine._refuse_if_wedged`, a plain
            # `RuntimeError`, which is the exact state the wedge exists to
            # report. Same verdict as the clause above, so the message is the
            # same: logged for the operator, never echoed to the caller.
            if cancelled.is_set():
