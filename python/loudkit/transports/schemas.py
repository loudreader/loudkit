...
class SpeakRequest(BaseModel):
    ...
    """A ``Literal``, so an unknown name is a 422 listing the four that work."""
...
class OpenAISpeechRequest(BaseModel):
    ...
    """Defaults to ``wav`` where OpenAI defaults to ``mp3``, because no mp3
    encoder ships here and honouring their default would answer every
    unconfigured client with an error."""
...
"""Refused by name. ``mp3`` and ``aac`` need an encoder this project does not
ship; ``opus`` is the trap: the ``ogg`` here is Vorbis, which shares
``audio/ogg`` with Opus and shares no bitstream with it."""

_STREAMABLE: frozenset[str] = frozenset({"wav", "pcm16", "flac"})
"""Formats a chunk can be delivered in on its own. Ogg's bitstream state spans
the whole stream, so a per-chunk container would lie about what the bytes are."""

_CODE_BY_STATUS = {
    400: "invalid_request",
    404: "invalid_request",
    # Starlette's router raises this one itself, for a path that exists under
    # another method. Without an entry it fell through to `server_fault`,
    # which tells a caller to retry a request that was never going to work.
    405: "invalid_request",
