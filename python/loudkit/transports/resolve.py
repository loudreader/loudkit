...
names, and where the voices beside it live. One answer, here, keeps the
three doors resolving alike, and ``tests/test_transport_resolution.py`` pins
it. Not a transport, like :mod:`.limits` and :mod:`.schemas`: what the peers
share. See ``docs/design/transports.md``.
...
into an :class:`~loudkit.config.AlgorithmConfig` before the engine loads,
and every door that offers the flag needs that done the same way.
"""
...
def open_release(
    ...
    release directory included, so that a directory's voices are looked for
