...
#: How long :func:`_publish_voice` keeps retrying a replace Windows refused,
#: and how long it waits between attempts. A second is far longer than a
#: contended rename or a scanner's read of a just-closed file, and far shorter
#: than a person waiting on `loudkit clone`.
_PUBLISH_RETRY_SECONDS = 1.0
_PUBLISH_RETRY_PAUSE = 0.02


def _publish_voice(tmp: Path, output: Path) -> None:
    """``os.replace``, with the retry Windows needs to behave like a rename.

    POSIX ``rename`` onto an existing path is atomic and cannot fail because
    somebody else is renaming onto the same path. Windows has no such call:
    ``MoveFileEx`` opens the destination to replace it, so a second writer
    publishing the same profile at that moment -- or a scanner still holding
    the file a beat after the first writer closed it -- is refused with
    ``ERROR_ACCESS_DENIED``, which reaches Python as ``PermissionError``.

    Repeating is safe: the temp is this caller's own and nothing else can name
    it, and a replace either happened or did not. A real permission fault
    still raises, one second later.
    """
    import time

    deadline = time.monotonic() + _PUBLISH_RETRY_SECONDS
    while True:
        try:
            ...
            if time.monotonic() >= deadline:
                raise
            time.sleep(_PUBLISH_RETRY_PAUSE)
...
Each is refused by name rather than parsed and dropped, so the server never
comes up silently running a setting the operator believes is in force.
``--first-chunk-tokens`` is the sharp one, because it changes the synthesis
algorithm on the transports that do honour it. HTTP drops none of them and
so is absent."""
...
def build_parser() -> argparse.ArgumentParser:  # noqa: PLR0915 - flat argparse declarations
    ...
    speak.add_argument(
        "--no-provenance",
        action="store_true",
        help="write a plain WAV, without the loudkit provenance manifest",
    )
    speak.add_argument(
