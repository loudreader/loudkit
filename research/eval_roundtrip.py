...
research/eval_floors.json (liaison invisible to WER, the pt variant undetectable,
...
Usage:
    python research/eval_roundtrip.py <checkpoint> <voice> out/
    # then transcribe out/*.wav with your ASR of choice and:
    python research/eval_roundtrip.py --score out/ transcripts.json
"""
...
FLOORS = REPO / "research" / "eval_floors.json"
...
def render(checkpoint: str, voice_path: str, out_dir: Path) -> None:
    ...
    for lang, items in probes.items():
        for i, item in enumerate(items):
            r = engine.synthesize(item["text"], voice, seed=7, language=lang)
