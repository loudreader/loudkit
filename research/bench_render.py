...
def _load_average() -> list[float]:
    """The run queue, where the platform reports one.

    `os.getloadavg` is POSIX. Windows has no equivalent in the standard
    library, and mypy checks this file for every platform the package
    supports, so a benchmark cannot name it unguarded over a field that only
    annotates a run.
    """
