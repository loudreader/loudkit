"""Run one full benchmark on one device: JSON plus every sample.

Produces leaderboard rows on a CUDA box, with the reproducing command,
plus every sample's tokens, mel and waveform saved to disk so the run is
re-inspectable. The parts the published rows were measured on are named in
docs/benchmarks.md.

Usage:
  python research/bench_cuda_box.py <checkpoint> <voice> <device> <outdir>
      [--seed N] [--cuda-graphs]
"""
...
def main() -> int:
    ...
    execution = ExecutionConfig(device=device, cuda_graphs=graphs) if graphs else None
    ...
    # run the benchmark
    cmd = (
        f"python tools/bench.py --checkpoint {ckpt} --voice {voice_path} --device {device} "
