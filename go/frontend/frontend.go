...
//
// An allowlist rather than a blacklist, because the tokenizer's vocabulary
// carries tags for 31 languages and only twelve have a frontend. A blacklist
// lets the rest through with the tag emitted, so Encode(text, "bg")
// NFKD-mangles Cyrillic into ids the model reads as sounds it was never
