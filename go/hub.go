...
func DownloadContext(ctx context.Context, repo, dir string, f Fetch) (string, error) {
	...
	// Counted, not assumed. The line used to print for every file the loop
	// walked, including a repo carrying no SHA256SUMS at all, where
	// verifyFetched returns before hashing anything: "verified 9 files against
	// SHA256SUMS" over a directory nothing had vouched for.
	if hashed > 0 {
...
func (c *hubClient) fetch(ctx context.Context, repo, revision, name, dest string, size int64) error {
	...
	// The listing already said how long this file is, and nothing else checks:
	// a body cut short by a dropped connection or a proxy was renamed into
	// place, hashed against nothing when the repo ships no SHA256SUMS, and
	// written into the receipt, so every later run trusted the receipt and
	// never looked again.
