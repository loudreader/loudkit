...
type Config struct {
	...
	// resume >= period: integer-exact, no constant to tune, and a cut that
	// survives it only ever removes a tail, like every other rule in the
	// layer.
	//
	...
	// decides what the resolver does with one that resumed.
	...
	// specimen that settled it is kathleen en0023 seed 1234: a mid-chunk
	// pause parked on ids 6486 (x7) then 6405 (x24), both rendering true
	// silence, both in the manifest's silence_render_ids, neither in the
	// sampler's silence_token_ids. Keyed to the sampler list, the exemption
	// could not see them: the 24-token period-1 run fired as a loop, and the
	// cut kept one cycle and deleted the pause plus the six seconds of
	// correctly-read speech behind it, two whole sentences, verdict
	// repetition, not suspect, no retry, audibly fluent. Six of the
	// checkpoint's eight truly-silent ids sit outside the sampler list, so
	// this was the rule's default behaviour on most real pauses; measured
	// prevalence 1/200 renders, and the shape is inaudible content loss
	// shipping as clean.
	//
...
//     stall the cap happened to interrupt (the reference specimen is 90.2%
//     silence, a 210-token run to the cap). This also closes a structural
...
//     so the rule that means "certainly broken" was unreachable by the most
//     broken rows this layer sees.
//
