...
//
// Five sections used to spell all eleven fields out for themselves, differing
// only in the one or two keys each adds. Eleven fields written five times is
// five places to mistype a json tag, and a mistyped tag reads as a zero rather
// than as an error, so the case runs against the wrong request and passes.
