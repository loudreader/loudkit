...
func Cardinal(value int64, language, gender string) (string, error) {
	...
	// `abs < 0` is the most negative int64 and nothing else: its positive
	// magnitude does not fit in int64, so negating it returns the same value.
	// Rejected here, with the values past the largest scale, before the
	// negative branch below can recurse on it.
...
// digit follows. Everything downstream reads the sign as `-`, so unfolded, a
// typographically correct minus is not a sign at all: it reaches the
// punctuation pass, becomes a space, and "−5" reads as *five*. Not U+2013,
...
func gluedForward(text string, end int) bool {
	...
	for i < len(text) {
		...
		// A thousands space: a digit in front of it and a group behind it, or
		// the walk leaves one number and enters the next and `1000 5.1e+3`
		// finds the `e` two tokens away and calls the whole line one glued
		// token.
		//
		// `startsAGroup` here and `continuesAGroup` backwards; see the two for
		// why the question is the looser one in this direction only. The
		// indices are the ones this direction needs: the digit sits at `i-1`, in
		// front of the space, and the group starts at `i+1`, behind it.
...
func gluedToAWord(text string, start int) bool {
	i := start
	for i > 0 {
		// Decoded backwards as a rune, not read as a byte, so a multibyte
		// letter is still a word boundary: `ł` is two bytes and neither is an
		// ASCII letter, so a byte-wise test walks past it and reads the `2,50`
		// in `zł2,50` that Python refuses.
		//
		...
		// A thousands space is crossed here too, so both ends of the whole
		// numeric run are tested for a letter glued to it. `C0200 000` binds as
		// one match, the lookbehind refuses it for the `C`, and the scan then
		// finds the standalone `000` with only a space behind it: crossing the
		// space reaches the `C`, and the whole thing stays written, as it does
		// in Python, JS and Swift.
		//
