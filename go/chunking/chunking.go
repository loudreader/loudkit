...
func SplitText(text string, cfg Config) []string {
	...
	for len(rest) > 0 {
		...
		if cut <= 0 {
			...
			// WordBoundaries, not U+0020: NBSP survives the funnel and is
...
// retracted with the harness that produced it, which seeded the second half
// from the next chunk's stream; the re-measurement compared only this law
// against no law, so the figures that once stood here have no un-retracted
