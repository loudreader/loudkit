...
func FromManifest(m map[string]interface{}) (AlgorithmConfig, error) {
	...
	if v, err := block(m, "chunking"); err != nil {
		...
		// Presence, not length, as for the abbreviations below. An empty
		// separator set used to fall back to the shipping five, which is the
		// splitting recipe of a manifest other than the one being read: the
		// text breaks in five places the manifest named none of, and the
		// canonical form records the shipping set as if the manifest had
		// declared it. chunking.Validate already carries the reference's
		// refusal for this, word for word; it was simply unreachable from a
		// manifest, because the fallback ran first.
	...
	// Every shape has been settled; what is left is whether the values make an
	// algorithm. Checked here rather than nowhere: chunking.Validate carries
	// the reference's refusals word for word and had one call site, in
	// engine.LoadWith, so a caller reading a manifest through this function
	// got none of them, and the sampling, window, grid and rate rules had no
	// call site at all.
...
// options. A present key ignored for having the wrong type is a law the
// manifest declared and this port did not run, and the canonical form then
// records the default it fell back to, so the fingerprint agrees with the
// misreading instead of reporting it. The reference arrives at the same
