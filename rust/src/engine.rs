...
impl EngineConfig {
    ...
    /// it, and until it could, every fingerprint check in this port compared a
    /// configuration written by hand, which holds even if the reader ignores
    /// every key and answers its defaults.
    ///
    /// # Errors
    ///
    ...
    ///
    /// None of them were here. Twelve manifests the reference and Swift both
    /// refuse loaded in this port and computed a fingerprint for an algorithm
    /// that cannot run: `n_cfm_timesteps: 0` makes the flow loop run zero times
    /// and renders the prior noise as audio, `token_rate_hz: 0` divides every
    /// duration by zero, `speech_vocab_size: 0` leaves no vocabulary, a start
    /// token past the embedding table indexes off the end, a start equal to the
    /// stop never stops, and a grid of the wrong length or the wrong direction
    /// integrates the ODE on a schedule that is not one.
    ///
    /// # Errors
    ///
...
impl Engine {
    ...
    pub fn synthesize_window(
        ...
        // README has promised the error since before 0.1.1.
        //
...
mod derive_seed_tests {
    ...
    ///
    /// `tests/weightfree.rs` pinned this by declaring `PHI` and `PSI` itself
    /// and recomputing the product, so it held the fixture to arithmetic
    /// written beside it rather than to the function every chunk seed in a
    /// real render goes through. Verified by mutation: changing the constant
    /// in `derive_seed` left that suite green. Go and JS had the same shape.
    ///
