...
impl Frontend {
    ...
    /// [`crate::numbers::supported_languages`] reports. An allowlist and not a
    /// blacklist of the five model-based ones, because the tokenizer's
    /// vocabulary carries tags for 31 languages: a blacklist lets the other 26
    /// through and emits the tag, so `encode(text, "bg")` NFKD-mangles
    /// Cyrillic into ids the model reads as sounds it was never trained to
    /// make. No error, plausible audio, wrong language.
    ///
    /// # Errors
