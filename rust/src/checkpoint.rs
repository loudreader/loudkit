...
fn chunking_from(manifest: &HashMap<String, Value>, nums: &mut Numbers) -> ChunkConfig {
    ...
    // Presence, not length. An empty separator set used to fall back to the
    // shipping five, which is the splitting recipe of a manifest other than the
    // one being read: the text breaks in five places the manifest named none
    // of, and the canonical form records the shipping set as if the manifest
    // had declared it. `ChunkConfig::validate` already carries the reference's
    ...
    // here, it is the old law spelled as data, so it must not fall back to the
    // shipping set the way an empty `split_on` used to.
...
mod tests {
    ...
    ///
    /// Two of the old zeros were not survivable: `speech_vocab_size` 0 sized
    /// the sampler's seen-token slice to nothing, and `n_cfm_timesteps` 0 left
    /// the Euler loop with no step to take, so the vocoder was handed the
    /// prior noise.
    #[test]
