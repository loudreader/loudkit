...
impl Config {
    ...
    ///
    /// Four ranges, refused at the manifest door before any of it reaches
    /// [`Sampler::call`] and the audio beyond it: a temperature of zero
    /// divides the logit row by zero; a `repetition_penalty` below 1.0
    /// rewards the repetition it exists to punish; a `min_p` of 1.0 or more
    /// masks every candidate including the argmax; a negative
    /// `min_tokens_text_ratio` pushes the end-of-speech floor below zero.
    ///
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        // A cap of zero decodes nothing, which reaches a caller as silence
        // they have to diagnose rather than an error they can read. A cap of
        // zero is not a configuration, it is a typo in a manifest, and every
        // port refuses it.
...
impl Sampler {
    ...
    pub fn call(&mut self, logits: &[f32], step: usize, seen: &[bool]) -> usize {
        ...
        if self.config.repetition_penalty != 1.0 {
            ...
            // exempt from it, and re-admitted below the min_p floor by the
            // exemption further down, a silence run has no exit. Penalising
            // seen silence is what lets the decoder leave a pause it has
            // entered. The min_p exemption is a separate rule. The
            // measurements are in `docs/design/sampler-and-noise.md`.
            for i in 0..n {
                if seen[i] {
                    z[i] = if z[i] > 0.0 {
