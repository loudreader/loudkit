...
pub fn expand_numbers(text: &str, language: &str) -> String {
    ...
    while let Some(caps) = DIGIT_RUN.captures_at(text, scan) {
        ...
        if sign {
            ...
            // still hold a number that is not. `e3 1000` binds as `3 100`, so
            // resuming after the refused match skips the rest of the region
            // and leaves the thousand behind it written. Python's engine
            // retries at every position, which is why it reads it.
            scan = whole.start() + 1;
            continue;
        }
        ...
        scan = end;
        // What `(?![\w])` reaches for, plus the backward walk its
        // one-character lookbehind cannot do.
        //
        // Both edges, and the whole identifier. Guarding the left alone
        // expands a run glued to a word on the right up to the letter and
        // then abandons it: "5x3" reads as *fivex3* and "1e6" as *onee6*, a
        // word welded to a digit. And a one-character lookbehind cannot see
        // past a dot between a letter and its digits, so "v1.2.3" starts its
        // scan at the `2` and reads "v1.two point three".
...
/// next search cannot start on: consuming the trailing boundary takes the
/// space that has to serve as the *following* occurrence's left boundary, and
/// every second occurrence is left written, so `etc. etc.` reads as *et cetera
/// etc.* Putting the trailing boundary character back before the next search
/// makes the match zero-width where the reference's is.
