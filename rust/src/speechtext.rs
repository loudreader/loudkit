...
/// compiling on demand would cost one compilation per mark per call to
/// [`speech_text`]. Every language is built at once, because the table is read
/// by language and a per-call miss would want a lock around it.
///
...
/// pattern has to *consume* the preceding character, because this engine has
/// no lookbehind, and a consumed character is one the scan cannot start the
/// next match at. `$1$2$3` then reads as `one dollars twenty-three dollars`:
/// the second `$` falls inside the first match's guard, the third amount
/// matches with `2` as its guard, and `2` and `3` are emitted next to each
/// other and spelled as one number nobody wrote.
/// `docs/design/text-funnel.md` is explicit that a confident wrong number is
...
/// Every number character becomes something a reader can say aloud, before
/// anything counts digits.
///
/// A non-ASCII decimal digit becomes the ASCII digit of the same value and
/// joins the run it was in, because a digit replacing a digit is still part of
/// that number. Every other numeral becomes the text `numerals.json` names for
/// it, separated from an adjacent letter or digit so `²9` does not fold into
/// `29`. The table decides *whether* a character is a numeral too: see
/// [`folded_numeral`].
///
/// Left in place, a superscript, a vulgar fraction or a circled numeral sits
/// inside the word as far as every "is this a word character" test goes, since
/// `\w`, `\p{N}` and `str.isdigit()` all admit `No`. The number matcher's
/// boundary guard then refuses the whole token and a bare digit reaches the
/// model, which a grapheme model reads badly with no way to hear that it
/// happened.
///
/// Python reference: `loudkit.frontend.speechtext.fold_numerals`;
/// `docs/design/text-funnel.md` carries the reasoning.
