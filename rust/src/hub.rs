...
///
/// `main` is this port's unpinned request. [`Options::revision`] is a `String`
/// whose default is `"main"`, so an omitted revision and an explicit one
/// arrive here as the same value; the reference tells them apart. Reading it
/// as a pin would refuse every default call over a cache fetched for a tag,
/// which is the commoner shape and the one no caller pinned, so it is read as
/// the absence it usually is.
#[cfg(feature = "download")]
fn refuse_a_cache_that_is_not_the_pin(
    root: &Path,
    receipt: &Receipt,
    revision: &str,
) -> Result<(), String> {
    ...
    if forty_hex {
        ...
        return Err(format!(
            "{}: holds {} at {}, and the hub cannot be reached to fetch {revision}. \
             A pinned revision is not a preference: rerun with the hub reachable, or \
             point the download directory at a copy of that commit.",
            root.display(),
            receipt.repo,
            receipt.commit
        ));
    ...
    Err(format!(
        "{}: was fetched for {:?} ({}), the hub cannot be reached, and {revision:?} \
         cannot be resolved without it. Rerun with the hub reachable, or ask for the \
         revision this copy holds.",
        root.display(),
        receipt.revision,
        receipt.commit
    ))
...
pub fn download_cancellable(
    ...
    let (commit, listing) = match revision_info(base, repo, revision) {
        ...
        Err(HubError::Unreachable(why)) => {
            ...
            refuse_a_cache_that_is_not_the_pin(&root, &receipt, &options.revision)?;
            eprintln!(
