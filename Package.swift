...
let package = Package(
    ...
    targets: [
        ...
        // The first mile, as something that compiles. `swift run Hello` fetches
        // the release, loads it and writes hello.wav, and `swift/README.md` is
        // the same source: a quickstart in a README rots, and one that is also
        // a target in this manifest cannot rot without the build going red.
        // Not a product, a package that depends on LoudKit does not build it.
