...
is end-to-end RTF; it does not isolate generator throughput. Model load is
separate. Python also accepts `coreml-hybrid` to measure the retained fp16
PyTorch generator with CoreML rendering.
...
Run one process at a time, after builds, exports and tests finish. The results
compare the ports on one machine only; do not use them for GPU or
cross-machine claims. Report the actual runtime versions and execution settings
with results.

From the repository root, create `out/` and build first:
...
The Swift link command is for Apple silicon; use the build directory for your
architecture on another Mac.
...
The Go and Swift runners look for `<name>.safetensors` inside a bundle
directory called `<name>`: `/models/loudr-1` must hold `loudr-1.safetensors`.
Quote a `TEXT` that contains spaces.
