...
of every stage, not just the final profile:
  ...
  wav16_flow.f32         the 10 s-capped clip at 16 kHz (the flow side's rate)
  wav16_t3.f32           the full clip at 16 kHz (the token-generator's rate)
...
The two 16 kHz clips are captured as separate files on purpose. One resampler
runs twice on *different inputs*: the flow side reads the 10 s-capped clip
(``_MAX_REF_SECONDS``, the length the static window holds), the token-generator
side reads the whole clip, because the utterance speaker embedding is a better
estimate over more audio. That is how the shipped voices were enrolled and what
``models/enroll.py`` does. A port that resamples once and reuses the result for
both cannot be byte-parity with the shipped voices.
...
from loudkit.models.enroll import _S3_SR, _matcha_mel
from loudkit.models.resample import resample
...
def main() -> None:  # noqa: PLR0915 - one stage per block, linear and logged
    ...
    # One resampler, two inputs: the capped clip and the whole clip.
    wav16_flow = resample(wav24, _MEL_SR, _S3_SR)
    wav16_t3 = resample(wav24_full, _MEL_SR, _S3_SR)

    tok = enroller._tokenizer  # the fixture owns the pipeline
    spk = enroller._speaker_encoder
    ve = enroller._voice_encoder
    ...
    tokenizer_mel = tok._log_mel(torch.from_numpy(wav16_flow)).numpy()
    ...
    voiceenc_trimmed, _ = librosa.effects.trim(wav16_t3, top_db=20)
    voiceenc_mel = ve._mel(voiceenc_trimmed.astype(np.float32))
    ...
    tokenizer_mel_cond = tok._log_mel(
    ...
    manifest = {
        ...
        "files": {
            ...
            "wav16_flow.f32": {
                "shape": list(wav16_flow.shape),
                "role": "24k->16k, capped clip",
            },
            "wav16_t3.f32": {"shape": list(wav16_t3.shape), "role": "24k->16k, full clip"},
            "tokenizer_mel.f32": {
