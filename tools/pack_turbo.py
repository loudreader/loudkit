...
def build_s3gen_tensors(
    ...
    `--n-cfm-timesteps` used to default to 2, so a K=1 estimator whose file
    records nothing packed without saying so produced exactly that: a
    checkpoint declaring 2, carrying a one-step estimator, and passing
    everything downstream including the CoreML exporter's own guard. There is
    no default now, an unrecorded K has to be stated, the way
    `export_coreml.py` already required.
    """
...
def main() -> None:  # noqa: PLR0915 - argparse builders read better flat
    ...
    # Written once. The payload hash belongs in the manifest that ships, and it
    # used to be read back off a first write for that reason: 1.3 GB written
    # twice. `tensor_payload_sha256` computes the same recipe over the tensors
    # still in memory, and the file form below proves the two agree, which is
    # the check the second write was standing in for.
