...
def main(source: Path) -> None:
    ...
    # Read everything before writing anything, and refuse an empty read.
    #
    # `Path.glob` on a path that is not a directory yields nothing rather than
    # raising, so a mistyped source used to reach the write below with zero
    # cases and replace the checked-in corpus with an empty one, printing
    # success. An import that found no CLDR data has nothing to say about the
    # fixture, so it says nothing.
