...
def _receipt_case(probe: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as scratch:
        ...
        if probe["receipt"] is not None:
            (root / hub.RECEIPT_NAME).write_text(json.dumps(probe["receipt"]), encoding="utf-8")
        if probe["sums"] is not None:
            (root / "SHA256SUMS").write_text(probe["sums"], encoding="utf-8")
...
def main() -> None:
    ...
    with open(OUT / vectors_name, "w", encoding="utf-8") as f:
