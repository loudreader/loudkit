...
def measure_backend(report: Report, checkpoint: Path, device: Device, label: str) -> None:
    ...
    # The thresholds are read from the fixture, not restated here: a gate
    # asserted in two places eventually means two things, and the copy in a
    # report generator is the one nobody re-measures.
...
def main() -> int:
    ...
    if args.out == "-":
        ...
        # `newline="\n"`: the table is checked in, so a run on one platform
        # must not rewrite every line of it for the next.
