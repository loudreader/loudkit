...
def _peak_rss() -> tuple[int, str]:
    ...
    if sys.platform == "win32":
        ...
        # bearing rather than good manners. A HANDLE is pointer-sized, and
        # ctypes defaults every unannotated function to `restype = c_int`, so
        # undeclared, `GetCurrentProcess()` comes back as a 32-bit -1 and is
        # passed on as 32 bits. psapi reads a pointer-sized argument, finds the
        # top half unset, and refuses the call with ERROR_INVALID_HANDLE, in
        # the last line of a run whose measurement has already been paid for.
