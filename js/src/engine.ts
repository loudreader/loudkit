...
export class Engine {
  ...
  async synthesizeWindow(
    ...
    // hears is fluent, complete-sounding and short. The package's README
    // documents this error as the contract for a text that does not fit.
    //
    ...
    //
    // Read the flag the generation captured, not the length of the returned
    // array: postprocess trims, so a window that filled and was then cut back
    // measures short in the returned tokens. Trimmed output length cannot say
    // whether generation hit the cap.
    if (hitWindow) {
