 ...
 *
 * `spec as { dtype, shape, data_offsets }` is a claim about JSON somebody else
 * wrote. Unchecked, a header missing `data_offsets` destructures `undefined`
 * and throws `TypeError: undefined is not iterable`, naming neither the file
 * nor the tensor; a spec that is a number, a string, a list or null throws
 * something similar; and `data_offsets: [0, 8, 9]` loads, silently keeping
 * the first two. The reference's reader refuses each of these by name.
 *
...
export class SafetensorsFile {
   ...
   * deciding whether to open it at all, and the alternative is a second copy
   * of this parser for want of an answer that does not read the payload.
   */
  static header(path: string): SafetensorsHeader {
    ...
    try {
      ...
      // own words. `Object.entries(5)` is the empty list, so unchecked, a
      // header holding a bare JSON number opens as a valid file with no
      // tensors in it, and one holding `null` raises a bare TypeError naming
    ...
    for (const [name, spec] of Object.entries(header)) {
      if (name === "__metadata__") {
        ...
        // holding an object rather than the JSON text of one, is dropped here
        // and reported as no embedded manifest, which is what every port
        // reports and what keeps `JSON.parse` reading text only.
        metadata = {};
        const block = spec;
