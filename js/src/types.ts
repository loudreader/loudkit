 ...
 *
 * Fourteen of the values this reader takes are counts: step counts, token ids,
 * sample rates, vocabulary sizes, window lengths. The reference reads every one
 * of them through `int()`; this port read them all as JSON numbers and kept the
 * fraction. `n_cfm_timesteps: 2.7` fingerprinted differently from the same
 * manifest read by the reference, and it also broke the schedule: `timeGrid`
 * answered `[0, 0.1645, 0.6039]` where two steps answer `[0, 0.2929, 1.0]`, so
 * the flow ODE stopped at t~0.60 and the audio was rendered from a
 * half-integrated state. No error, plausible audio, wrong.
 */
function intAt(
  block: Record<string, unknown>,
  key: string,
 ...
 * check. It read `null` as absent, so `"chunking": null`, `"sampling_defaults":
 * null` and `"eos_floor": null` each silently took the shipping defaults where
 * the reference refuses the file; and it read a list or a scalar as an object,
 * so `"window": [1, 2, 3]` silently rendered the ragged window, which
 * `go/config/config.go`, `rust/src/checkpoint.rs` and
 * `swift/LoudKit/ManifestReader.swift` each say in their own words must never
 * happen. A block named in a manifest and ignored here is a law the file
 * declared and this port did not run, and the canonical form then records the
 * default it fell back to, so the fingerprint agrees with the misreading
 ...
 *
 * Presence, not length, and not `Array.isArray` either. Both of the chunking
 * lists used to fall back to the shipping recipe whenever they did not arrive
 * as a non-empty list of strings, which made three refusals unreachable from a
 * manifest: `split_on: []`, which `validateChunkConfig` refuses because there
 * would be nowhere to break, and a `split_on` or `abbreviations` given as a
 * bare string, which the reference refuses by name because a string is a
 * sequence of characters. Go and Rust describe this as the bug they fixed.
 */
...
function postprocessFromManifest(manifest: Record<string, unknown>): PostprocessConfig {
  ...
  for (const [field, key, kind] of numeric) {
    ...
    // of it: this loop checked the fraction and not the exactness.
    (cfg[field] as number) =
...
function guidanceFromManifest(manifest: Record<string, unknown>): GuidanceMode {
  // Presence, not `??`: an explicit `null` is a declared value this port cannot
  // read, and reading it as an absent key rendered single-path audio for a
  // manifest that named something else. The reference renders it (`str(None)`)
  // and refuses what it rendered.
 ...
 * grid with a hole in it integrated the ODE on a schedule that started where
 * the manifest did not, and the canonical form then recorded that schedule.
 */
 ...
 *
 * None of them were here. A manifest the reference and three other ports all
 * refuse loaded in this port and computed a fingerprint for an algorithm that
 * cannot run: `n_cfm_timesteps: 0` makes the flow loop run zero times and
 * renders the prior noise as audio, `token_rate_hz: 0` divides every duration
 * by zero, `speech_vocab_size: 0` leaves no vocabulary, a start token past the
 * embedding table indexes off the end, a start equal to the stop never stops,
 * and a grid of the wrong length or the wrong direction integrates the ODE on
 * a schedule that is not one.
 *
...
export function decodeFromManifest(manifest: Record<string, unknown>): "single" | "fusion_mtp2" {
  // `manifest.decode as Record | undefined` was a cast: a list or a scalar read
  // as a block, answered `undefined` for `.mode`, and decoded single-path under
  // a manifest naming a loop this port never looked at. `decode: null` is the
...
export function algorithmFromManifest(manifest: Record<string, unknown>): AlgorithmConfig {
  ...
  // `(manifest.window ?? {})` could not do: it rendered the ragged window for
  // `window: [1, 2, 3]` and then recorded that window in the canonical form, so
  // the fingerprint agreed with the misreading.
  ...
  // are missing, and Swift does the same. Filling 255/4254/238 here made the
  // same manifest fingerprint differently in this port, which is the
  ...
  const cfg: AlgorithmConfig = {
    ...
    // that names it null says the same thing out loud. This port was the only
    // one of the five that refused the explicit spelling.
