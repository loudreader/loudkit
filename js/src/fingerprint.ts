...
function jsonStr(s: string): string {
  ...
  for (const ch of s) {
    ...
    // astral one is a surrogate pair. A raw UTF-8 spelling hashes differently
    // from the reference, which is invisible until a hashed string carries a
    // non-ASCII character.
 ...
 *
 * Not sorted. The reference decides the canonical form, and it sorts the
 * *keys* of every mapping while rendering every list as it stands:
 * `AlgorithmConfig.canonical_form`, and Rust and Swift with it. Sorting here
 * makes a manifest declaring `[9, 1, 5]` hash to the number Python answers
 * for `[1, 5, 9]`, so two engines computing different things would agree on
 * one fingerprint.
 *
 * Nothing sorts on the way in either, so the order a census is declared in is
 * the order the sampler and the detectors read it in.
 */
