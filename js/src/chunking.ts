 ...
 * exist for. This block claimed "an overflow is a hard failure" after the
 * Python reference had retracted it.
 *
...
export function splitText(text: string, config: ChunkConfig): string[] {
  ...
  while (rest.length > 0) {
    ...
    if (cut <= 0) {
      ...
      // in real prose, so text whose every space is non-breaking found no
      // boundary here and got cut mid-word.
      let at = -1;
 ...
 * because chunk texts are fixed before any of them is rendered. Measured across ten
 * languages, 54 of 9920 chunks reached a cap and 30 were still speaking when it
 * closed, over five voices; gating on the window leaves 51 and 27, on two.
 *
 ...
 * the same mechanism as every other defect in this campaign, arriving through
 * the chunker. The arm that measured punctuation against word boundaries was
 * retracted with the harness that produced it (it seeded the second half from
 * the next chunk's stream), and the re-measurement compared only this law
 * against no law, so the numbers that once appeared here have no un-retracted
 * source. Not sought is not avoided: the nearest word break can follow a
 * comma.
 *
