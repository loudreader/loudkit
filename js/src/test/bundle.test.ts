 ...
 *
 * The three paths `Engine.load` used to ask the caller for are all derivable
 * from the directory the download wrote, and the interesting cases are the
 * ones where they are not there: a half-fetched set has to fail here, naming
 * the file, rather than six graphs later inside onnxruntime.
 */
