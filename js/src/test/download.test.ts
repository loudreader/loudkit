...
test("two downloads into one directory do not take the file from each other", async () => {
  // Both wrote `<target>.part` and both renamed it onto the target: the first
  // rename took the file out from under the second, which failed with
  // `ENOENT ... rename loudr-1.safetensors.part`. Two workers starting at once
  // on a cold cache reach it, with no broken model and no hostile input.
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [],
      "neither call may fail"
    );
    assert.equal(readFileSync(join(dir, "loudr-1.safetensors"), "utf8"), "checkpoint bytes");
    assert.equal(
      readdirSync(dir).filter((n) => n.endsWith(".part") || n.endsWith(".lock")).length,
      0,
      "no part or lock file survives"
    );
  } finally {
    await stub.close();
  }
...
test("two downloads race over the set, not just over one file", async () => {
  // A third-party repo without SHA256SUMS: nothing is kept by hash, so every
  // file is fetched and both callers replace the whole set at once. The unit
  // that has to be exclusive is the directory, because that is the unit the
  // inventory check speaks of. Third-party rather than official, because an
  // official repo without SHA256SUMS is refused before any file moves, which
  // is a different rule.
  const stub = await hub(stubRelease());
  const dir = workdir();
  const stranger = "someone/loudr-1";
  try {
    const results = await Promise.allSettled([
      download(stranger, dir, { endpoint: stub.endpoint, ...quiet }),
      download(stranger, dir, { endpoint: stub.endpoint, ...quiet }),
    ]);
    assert.deepEqual(
    ...
    assert.ok(existsSync(join(dir, "tokenizer.json")), "the set is complete");
...
test("a lock that is still being held is waited for, not taken", async () => {
  // A held lock is waited for rather than broken, and the wait ends when the
  // lock is released. Liveness is the heartbeat, not a deadline: a checkpoint
  // download takes longer than any fixed timeout worth choosing, so only a lock
  // that stops being touched is treated as abandoned.
  const dir = workdir();
  const lock = `${dir}.lock`;
  writeFileSync(lock, "");
  ...
  const stub = await hub(stubRelease());
  try {
    const held = download("someone/loudr-1", dir, { endpoint: stub.endpoint, ...quiet });
    // Kept alive the way a working holder keeps it alive.
    ...
    const raced = await Promise.race([
      held.then(() => "finished"),
      new Promise((resume) => setTimeout(() => resume("still waiting"), 1500)),
    ]);
    clearInterval(beat);
    assert.equal(raced, "still waiting", "a held lock must not be taken over");
    rmSync(lock, { force: true });
    assert.equal(await held, dir, "and the download proceeds once it is released");
  } finally {
    await stub.close();
  }
});
...
test("nothing is claimed verified when the repo ships no SHA256SUMS", async () => {
  // A counter that rises for every file in the plan reports "verified against
  // SHA256SUMS" over a repo with no manifest, where `verifyFetched` returns
  // from its first line having hashed nothing. That sentence is what a caller
  // reads to decide whether the bytes are the release's.
  const files = stubRelease();
  const stub = await hub(files);
  const dir = workdir();
  const lines: string[] = [];
