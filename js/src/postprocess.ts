...
export interface PostprocessConfig {
   ...
   *
   * 1.7 sits inside the measured gap between a mute keep and a real one, and
   * deliberately under 1.75, the floor of the healthy band of speech tokens
   * per text token, so a complete read is never condemned. Cap-hit rows only:
   * a row that ended on its own corroborated its trim with a stop token. Zero
   * disables the trigger. The specimen and the calibration rows are in
   ...
   * this field closes it on every checkpoint, including ids no census lists.
   *
   * Not a discard-fraction guard: a fraction reads geometry, so a pause with
   * less speech behind it slips under any cap and ships the deletion as
   * clean. The largest resume a truncated genuine loop can produce is
   ...
   * field decides what the resolver does with one that resumed. The specimen
   * is in `docs/design/postprocess-detectors.md`.
   */
   ...
   * *any* silent-rendering id is never mistaken for a decoder loop. Keyed to
   * the sampler list alone, the exemption cannot see a pause parked on ids
   * that render true silence but sit outside that list: the run fires as a
   * loop, and the cut keeps one cycle and deletes the pause together with the
   * correctly-read speech behind it, verdict repetition, not suspect, no
   * retry, audibly fluent. Most of the measured checkpoint's truly-silent ids
   * sit outside the sampler list, so this is the rule's behaviour on most
   * real pauses, and the shape is inaudible content loss shipping as clean.
   *
 ...
 *   the cap happened to interrupt. This also closes a structural hole: the
 *   ceiling clips rows to 4.0x text tokens + 40, so past 80 text tokens a
 *   cap-hit stall can never reach the 4.5x desperation threshold, which puts
 *   the rule that means "certainly broken" out of reach of the most broken
 *   rows this layer sees.
 *
 ...
 *
 * One resolver rather than a detector per call site, with the precedence
 * written down here: an order that lives in a caller is an order the next
 * caller gets wrong.
 *
