 ...
 * was made by.
 *
 * A profile whose header names a strategy this build does not implement was cut
 * from its recording a different way, so loading it here would speak in a
 * different voice under the same name. Python and Go refuse it; this port and
 * two others loaded `"first-30s-nonsense"` and spoke.
 */
...
export function loadVoice(path: string): VoiceProfile {
  ...
  // when it is another, rather than stringified: `language: 5` was the
  // language id "5", and language selects the text funnel, so a voice whose
  // header says 5 was read through whichever funnel the coercion landed on.
  // An absent key takes the default, which is the branch every profile written
  // before a field existed goes down.
  ...
  // The declared shape, not the element count. Both say something, and only
  // the count was being read: `prompt_mel` declared [2, 80] holds the same 160
  // floats as [80, 2] and means the transpose of them, so the profile loaded
  // and the voice was read on its side. A vector declared [16, 16] or [1, 256]
  // is 256 floats too, and the reference refuses all of these by rank.
