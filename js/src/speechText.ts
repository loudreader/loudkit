...
function foldNumerals(text: string): string {
  ...
  for (let i = 0; i < chars.length; i += 1) {
    ...
    // The last *code point* already emitted, not the last UTF-16 unit: for a
    // character above the BMP the unit is a lone surrogate, which `\p{L}` does
    // not match, so the space was not inserted and the numeral glued itself to
    // an astral letter.
 ...
 * start walks out of the block where two blocks touch, since `MATHEMATICAL
 * DOUBLE-STRUCK DIGIT ZERO` follows `MATHEMATICAL SANS-SERIF DIGIT NINE` with
 * nothing between, and NFKC does not reach `ETHIOPIC NUMBER TEN`, the Aegean
 * numbers, the Kaktovik digits or the Meroitic numerals, which then vanished;
 * and asking `\p{N}` *whether* to fold put a Unicode version back into an answer
 * the table had taken it out of. V8's tables move with the runtime, Python's
 * with the interpreter, and the five ports report one fingerprint. The table is
 * cut from one pinned UCD and hashed into the grammar digest.
 *
