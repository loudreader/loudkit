 ...
 * one: it leaves a repo carrying both loops fetched without `t3_step.onnx`,
 * and then declares that cache complete. `synthesisGraphs` is where the loop
 * does decide, at the two places that open the graphs.
 */
...
/**
 * Run `body` with this release directory to itself.
 *
 * The directory, not the file. A per-file lock still lets two downloads race
 * over the set, one clearing a file the other has already published and is
 * hashing. The unit that has to be exclusive is the one the receipt and the
 * inventory check speak about, and that is the directory.
 *
 * A lock file rather than a mutex, because the cache is shared between
 * processes. `wx` is the atomic part: the file is created or the open fails,
 * with no window between asking and having. It lives beside the directory
 * rather than inside it, so no inventory check has to know about it.
 *
 * A holder that dies must not block the next caller forever, and a holder that
 * is simply slow must not be robbed: a 747 MB checkpoint takes longer than any
 * fixed timeout worth choosing. So the holder touches the lock every
 * `LOCK_BEAT_MS` while it works, and a waiter claims it once the mtime has
 * stood still for `LOCK_ABANDONED_MS`. A lock that has stopped being touched
 * is a heuristic for an abandoned holder, not proof that the process died: a
 * holder stopped long enough by a suspended machine or a paused process loses
 * the lock while still alive, so the window is wide enough that ordinary
 * slowness never reaches it.
 */
const LOCK_BEAT_MS = 5_000;
const LOCK_ABANDONED_MS = 30_000;

async function withReleaseLock<T>(
  dir: string,
  signal: AbortSignal | undefined,
  body: () => Promise<T>
): Promise<T> {
  ...
  for (;;) {
    ...
    try {
      ...
      try {
        ...
        // Released between the failed open and the stat: ask again.
        continue;
      }
  ...
  const beat = setInterval(() => {
    try {
      ...
      // The lock is ours; if it is gone there is nothing to keep alive.
    }
  }, LOCK_BEAT_MS);
  try {
    return await body();
 ...
 * when the listing did not say. Three things turn on it:
 *
 ...
 *  - The bytes written are compared against it, or a short body is renamed
 *    over the target and cached and the next run trusts the receipt.
 *    `SHA256SUMS` catches that for a release that ships one and lists the
 *    file; a stranger's repo ships neither, and `SHA256SUMS` cannot vouch for
 *    itself.
 *
 * The transport is wrapped so a broken connection names the file and the URL,
 * where undici's bare `TypeError: terminated` names neither.
 */
 ...
 * when a connection drops, which names neither, leaving a caller two words
 * and no way to tell which file of the set broke. An abort passes through as
 * itself: the caller asked for it and knows what they cancelled.
 */
...
export async function download(
  ...
  if (commit === null) {
    ...
    if (receipt && complete(dir, options.cloning)) {
      // A pin is not a preference: handing back whatever is cached and
      // naming it in a message lets a caller asking for one revision run on
      // another and be told it succeeded. Forty hex names the commit the
      // receipt records; anything else is a ref, which cannot be resolved
      // with no hub, and the receipt records which one this copy answers for.
      if (options.revision) {
        const asked = options.revision;
        const holds = /^[0-9a-f]{40}$/.test(asked) ? receipt.commit : receipt.revision;
        if (asked !== holds) {
          throw new Error(
            `${dir}: holds ${repo} at ${receipt.revision} (${receipt.commit}), the hub at ` +
              `${endpoint} cannot be reached, and ${asked} cannot be answered from it. ` +
  ...
  return withReleaseLock(dir, options.signal, async () => {
    // Another process may have finished this very release while we waited.
    if ((await receiptHit(dir, repo, commit, options.cloning)) && complete(dir, options.cloning)) {
      return dir;
    }
    return fetchRelease(repo, dir, revision, commit, endpoint, token, say, options);
  });
}
...
async function fetchRelease(
  ...
  for (const [index, path] of bookkeepingFirst(wanted).entries()) {
    ...
    // Counted only when something was actually hashed. `verifyFetched`
    // returns without touching a file when the repo has no `SHA256SUMS` at
    // all, and when the manifest does not list that file, so counting the
    // plan would report a stranger's unvouched-for repo as verified. `kept`
    // counts, because `keptByHash` hashed it.
