 ...
 *
 * The list is not a place to be economical. `numerals.json` was added to the
 * funnel and not to this array, and because data/ is gitignored, every
 * developer machine kept working from the copy an earlier build had left
 * there: `npm test` was green here and `ENOENT` on a fresh `git archive`.
 * `check-pack.mjs` names each file for the same reason.
 *
