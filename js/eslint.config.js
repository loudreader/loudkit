// The checks the type system does not make: an unused import, a floating
// promise, a `==` where a `===` was meant.
//
...
export default tseslint.config(
  ...
  },
  {
    ...
    // `no-floating-promises` fires on every `test(...)` in the suite. Off
    // here and on in `src/`, which is where a dropped await is actually a
    // bug: the engine is async and its callers await it.
    ...
    rules: {
      ...
      // honest annotation; `no-unused-vars` stays on.
      "@typescript-eslint/no-explicit-any": "off",
      "@typescript-eslint/no-unsafe-call": "off",
      "@typescript-eslint/no-unsafe-return": "off",
    },
